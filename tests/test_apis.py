...
DEALER_NODE_API_URL = "https://friendly-giggle-r44gjqwqjjxwfxj7-3030.app.github.dev"
CARS_NODE_API_URL = "https://friendly-giggle-r44gjqwqjjxwfxj7-3050.app.github.dev"
...
def test_dealer_mongodb_api():
    ...
    # Ensure every dictionary in the response has 'state' key with value 'Kansas'
    assert all(dealer.get("state") == "Kansas" for dealer in json_data if isinstance(dealer, dict)), \
...
def test_carsinventory_mongodb_api():
    ...
    assert all(car.get("dealer_id") == 1 for car in json_data if isinstance(car, dict)), \
